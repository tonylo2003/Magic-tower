...
#define tag magic_tower_level1_crafting_particle

#激活
execute as @e[type=item,nbt={Item:{id:"minecraft:diamond",Count:1b}}] at @s positioned ~ ~-1 ~ unless entity @e[distance=..1,tag=magic_tower_level1_core_synthesis_table] if predicate magic_tower:synthesis_table/level1_core_synthesis_table at @s if entity @e[distance=..0.5,nbt={Item:{id:"minecraft:copper_ingot",Count:10b}}] align xyz positioned ~0.5 ~-1 ~0.5 run function magic_tower:synthesis_table/level1_core_synthesis_table/activating
#特效
...
#合成特效
execute as @e[tag=magic_tower_level1_crafting_particle] at @s run function magic_tower:synthesis_table/level1_core_synthesis_table/crafting_particle
#破壞
